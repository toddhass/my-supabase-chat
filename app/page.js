...
export default function ChatApp() {
  // 1. Initialize with a static default to prevent hydration errors
  ...
  // 2. Handle localStorage and random name generation ONLY on the client
  useEffect(() => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const uniqueNames = [...new Set(parsedMsgs.map((m) => m.username))].filter(
        (n) => n !== username
      );
      ...
      .on(
        ...
        (payload) => {
          if (payload.new.content.startsWith("[" + currentRoom + "]")) {
            ...
            setRecentUsers((prev) => {
              if (!prev.includes(newMsg.username) && newMsg.username !== username) {
  ...
  return (
    <div className="flex h-screen bg-slate-950 text-slate-100 overflow-hidden">
      {/* Sidebar */}
      <aside
        className={`fixed md:relative z-40 h-full md:h-auto ${
          sidebarOpen ? "translate-x-0" : "-translate-x-full"
        } md:translate-x-0 w-72 md:w-80 transition-transform duration-300 bg-slate-900 border-r border-slate-800 flex flex-col`}
      >
        <div className="p-6 font-black text-2xl tracking-tighter text-emerald-500">
          SUPA-CHAT
        </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="h-14 md:h-16 px-3 md:px-6 border-b border-slate-800 flex items-center justify-between bg-slate-900/50">
          <button
            ...
            className="text-emerald-500 text-xl md:hidden"
          >
            ☰
          </button>

          <h2 className="font-bold text-base md:text-lg">#{currentRoom}</h2>
            ...
            return (
              ...
              >
                ...
                <div
                  className={
                    isMe
                      ? "bg-emerald-600 p-3 rounded-2xl rounded-tr-none text-sm max-w-[80%]"
                      : "bg-slate-800 p-3 rounded-2xl rounded-tl-none text-sm max-w-[80%]"
                  }
                >
                  {msg.text}
                </div>
        ...
        <footer className="sticky bottom-0 p-2 md:p-4 bg-slate-900 border-t border-slate-800">
          <form onSubmit={sendMessage} className="flex gap-2">
            ...
            />
            <button className="bg-emerald-600 px-4 md:px-6 rounded-lg text-xs font-bold text-white">
              Send
            </button>
          </form>
        </footer>
